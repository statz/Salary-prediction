...
if not os.path.exists("data\\train_x2.h5"):
    print(1223)
    text = train_data["TextOfVacancy"].astype(str)
    n_feat, batch_size, min_df, ngr = [2000, 50000, 50, (1, 2)]
    tfidf = CountVectorizer(min_df=min_df / len(text), max_features=n_feat, binary=True, ngram_range=ngr)
    tfidf.fit(text)
    h5f = h5py.File("data\\train_x"+str(2)+".h5", 'w')
    ...
    h5f = h5py.File("data\\test_x"+str(2)+".h5", 'w')
    h5f.create_dataset('dataset_1', data=tfidf.transform(test_data["TextOfVacancy"].astype(str)).toarray())
    h5f.close()
...
if not os.path.exists("data\\train_x4.h5"):
    ...
    tfidf = CountVectorizer(max_features=1000, binary=True, ngram_range=(1, 1))
